...
namespace GorillaNetworking
{
	public class PlayFabAuthenticator : MonoBehaviour
	{
		...
		public void Awake()
		{
			PhotonNetwork.PhotonServerSettings.AppSettings.AppIdRealtime = "8e408a57-bc61-4a85-b316-671d3692f887";
			PhotonNetwork.PhotonServerSettings.AppSettings.AppIdVoice = "9b3586d6-236a-4b66-b163-02a3c47152d1";
			PhotonNetwork.PhotonServerSettings.AppSettings.AppVersion = "BASE_TAG";
			PlayFabSettings.TitleId = "BB7FB";
			if (instance == null)
			{
				instance = this;
			}
			...
			if (obj.ErrorMessage == "The account making this request is currently banned")
			{
				using (Dictionary<string, List<string>>.Enumerator enumerator = obj.ErrorDetails.GetEnumerator())
				{
					if (enumerator.MoveNext())
					{
						...
						{
							gorillaComputer.GeneralFailureMessage("YOU HAVE BEEN BANNED from Base Tag. YOU WILL NOT BE ABLE TO PLAY UNTIL THE BAN EXPIRES.\nREASON: " + current.Key + "\nHOURS LEFT: " + (int)((DateTime.Parse(current.Value[0]) - DateTime.UtcNow).TotalHours + 1.0));
						}
						else
						{
